...
8. 你有 send_file_to_wechat 工具可以将电脑上的图片发送到微信端，用户要求把图片发到微信时记得使用。需要输出完整 file_path 字段（如 "~/Desktop/xxx.png"），代码会自动执行发送
...
- 需要查资料/处理文件时尽管用工具
- 你有 send_file_to_wechat 工具可以把本地图片发给对方微信，用户要求发图/发文件时用。JSON 中加 file_path 字段（如 "~/Desktop/img.png"）

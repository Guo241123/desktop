...
14.  【语音工具】你已接入电脑麦克风和语音合成：
    ...
    - text_to_speech(text) — 把文字通过电脑扬声器朗读出来！
    **重要：当用户提到「语音」「说话」「出声」「朗读」「念出来」「声音」「说给我听」等字眼时，你必须在回复中设置 tool="text_to_speech"！**
    **聊天中如果你想用语音回复，也设置 tool="text_to_speech"，系统会朗读你的回复文字。**
    **文本回复和语音朗读同时进行，两者不冲突。**
...
18.  【使用技能】当用户说"用xxx技能帮我做xxx"时，你应该这样做：
   ...
   ④ 你负责转述 Pro 的结果给用户
   ⑤ 当前已安装技能：frontend-slides-editable（生成可编辑HTML幻灯片）、my-coffee（瑞幸咖啡下单助手，查门店/商品/下单/查取餐码）
"""
...
- 【语音工具】你已接入电脑麦克风和语音合成：
  ...
  - text_to_speech(text) — 把文字通过电脑扬声器朗读出来！
  **重要：当用户提到「语音」「说话」「出声」「朗读」「念出来」「声音」「说给我听」「能不能说话」等相关字眼时，你必须在回复JSON中设置 tool="text_to_speech"！**
  **当你在聊天中想用语音回复用户时，也设置 tool="text_to_speech"，系统会自动朗读你的回复文字。**
  **注意：文本回复会正常发送到微信/桌面，同时语音也会从电脑喇叭播放出来，两者不冲突。**
